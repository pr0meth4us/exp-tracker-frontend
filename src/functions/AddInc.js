...
export default function AddInc() {
    ...
    const onInputChange = e => {
        ...
        const formattedDate = name === 'date' ? new Date(value).toISOString().slice(0, 19).replace("T", " ") : value;

        setIncome({ ...income, [name]: formattedDate });
    }
    const onSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post("http://localhost:8080/inc", requestBody);
    ...
    return (
        <div className='container'>
            <div className='col-md-6 offset-md-3 border rounded p-4 mt-2 shadow row'>
                <h1 className='text-4xl font-bold'>Add Income</h1>
